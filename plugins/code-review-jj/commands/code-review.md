---
allowed-tools: Bash(gh issue view:*), Bash(gh search:*), Bash(gh issue list:*), Bash(gh pr comment:*), Bash(gh pr diff:*), Bash(gh pr view:*), Bash(gh pr list:*), Bash(jj log:*), Bash(jj status:*), Bash(jj diff:*)
description: Code review a pull request
argument-hint: "PR number or URL (optional — auto-detects open PRs)"
...
4. Then, launch 5 parallel Sonnet agents to independently code review the change. The agents should do the following, then return a list of issues and the reason each issue was flagged (eg. CLAUDE.md adherence, bug, historical git context, etc.):
   ...
   c. Agent #3: Read the file annotation history (use `jj file annotate`, NEVER `git blame` or `git log`) of the code modified, to identify any bugs in light of that historical context
...
**Non-colocated jj repos**: If using a non-colocated jj repository (no `.git/` directory), ensure `GIT_DIR=.jj/repo/store/git` is set for `gh` CLI, or run `jj git export` before using this command. In colocated repos (`.jj/` + `.git/`), `gh` works directly with no extra configuration.
